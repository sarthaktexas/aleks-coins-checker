 ...
 * Dispatches the GitHub Actions "ALEKS check login" workflow (workflow_dispatch).
 * Professors only.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const workflowFile = process.env.ALEKS_CHECK_LOGIN_WORKFLOW || "aleks-check-login.yml"
    ...
    if (res.status === 204) {
      return NextResponse.json({
        success: true,
        message:
          "ALEKS login check started. Open GitHub → Actions to watch the run and see if the stored credentials still work.",
        repo,
    ...
    return NextResponse.json(
      {
        error: "Failed to trigger ALEKS login check",
