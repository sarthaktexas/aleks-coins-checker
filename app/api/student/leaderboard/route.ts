...
export async function GET(request: NextRequest) {
  try {
    ...
    // Validate input
    if (!studentId || !period || !sectionNumber) {
      return NextResponse.json({ error: "studentId, period, and sectionNumber are required" }, { status: 400 })
    }
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ error: "No data found for this period and section" }, { status: 404 })
    }
    ...
    try {
      ...
      // Only include period-specific adjustments (not redemptions)
    ...
    // Find the current student's rank (1-based)
    const studentIndex = studentsWithCoins.findIndex(s => s.studentId === normalizedId)
    const studentRank = studentIndex >= 0 ? studentIndex + 1 : null

    // Get #1 person's coin count
    const topStudentCoins = studentsWithCoins.length > 0 ? studentsWithCoins[0].totalCoins : 0

    return NextResponse.json({
      success: true,
      rank: studentRank,
      topStudentCoins,
      totalStudents: studentsWithCoins.length
    })
  } catch (error) {
